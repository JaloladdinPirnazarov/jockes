...
class UIComponents {
  ...
  Widget buildListViewItem(JokeModel joke, int position, bool isAllJokes, BuildContext context) {
    return InkWell(
      onTap: (){
  ...
  void jokeInfoAlert(JokeModel jokeModel, BuildContext context){
    showDialog(context: context, builder: (context)=>AlertDialog(
      ...
      content: Column(
        ...
        children: [
          Text("${'setup'.tr}: ${jokeModel.setup}", style: TextStyle(color: Colors.black, fontSize: 18),),
          SizedBox(height: 10,),
          Text("${'punchline'.tr}: ${jokeModel.punchline}", style: TextStyle(color: Colors.black, fontSize: 18),),
          Divider(height: 40,thickness: 2,),
          Text("${'type'.tr}: ${jokeModel.type}", style: TextStyle(color: Colors.black54, fontSize: 16),),
          Text("ID: ${jokeModel.id}", style: TextStyle(color: Colors.black54, fontSize: 16),),
        ],
      ),
      actions: [
